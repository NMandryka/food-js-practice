...
function calculator() {
    ...
    function getDataFromStaticEl(parentSelector, activeClass) {
    ...
    getDataFromStaticEl('#gender', 'calculating__choose-item_active');
    getDataFromStaticEl('.calculating__choose_big', 'calculating__choose-item_active');

    getDataFromInput('#age');
    getDataFromInput('#height');
    getDataFromInput('#weight');
...
function modal(closeModalSelector, openModalSelector, modalSelector, modalTimerId) {

    const modalOpenBtn = document.querySelectorAll(openModalSelector),
          modalCloseBtn = document.querySelector(closeModalSelector),
...
function slider({container, nextArrow, prevArrow, slide, totalCounter, currentCounter, wrapper, field}) {
    ...
    next.addEventListener('click', () => {
        if (offset == (getDigitsFromString(width) * (slides.length - 1))) {
            offset = 0;
        } else {
            offset += getDigitsFromString(width); 
        }
        ...
        shouldAddZeroBeforeNumberInCurrentSlide();
    ...
    prev.addEventListener('click', () => {
        if (offset == 0) {
            offset = getDigitsFromString(width) * (slides.length - 1);
        } else {
            offset -= getDigitsFromString(width);
        }
    ...
    dots.forEach(dot => {
        dot.addEventListener('click', (e) => {
            ...
            slideIndex = slideTo;
            offset = getDigitsFromString(width) * (slideTo - 1);
    ...
    function shouldAddZeroBeforeNumberInCurrentSlide() {
        if (slides.length < 10) {
            current.textContent =  `0${slideIndex}`;
        } else {
            current.textContent =  slideIndex;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    (0,_modules_modal__WEBPACK_IMPORTED_MODULE_3__["default"])('[data-modal-close]', '[data-modal]', '.modal', modalTimerId);
    (0,_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('.timer', '2022-11-15');
